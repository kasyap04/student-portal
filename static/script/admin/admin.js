...
function openDepMenu(t, e) {
    e.stopPropagation() ;
    $(".menu-dep").show() ;
    $(".menu-dep").css({
        "top": $(t).offset().top + "px",
        "left": $(t).offset().left - $(".menu-dep").width() + "px"
    }) ;
}
...
function openEditSubMenu(t, e) {
    ...
    $(".menu-subject a:nth-child(1)").attr('href', `/subject/addSubject?dep=${dep}&sem=${sem}`) ;
    $(".menu-subject a:nth-child(2)").attr('href', `/subject/editsubject?dep=${dep}&sem=${sem}`) ;
    $(".menu-subject a:nth-child(3)").attr('href', `/fees/editfee?dep=${dep}&sem=${sem}`) ;
    $(".menu-subject a:nth-child(4)").attr('href', `/notifications/add?dep=${dep}&sem=${sem}`)  ;
}


function closeUpgradeSemCont() {
    $(".upgradeSem-outer").hide() ;
}


function openUpgradeSemCont() {
    document.getElementById('upgradeDep').innerText = $(".upgradeSem-outer").attr('data-depName') ;
    $(".upgradeSem-outer").show() ;
}

function upgradeSemCount() {
    let depID = $(".upgradeSem-outer").attr('data-depId') ;
    let depName = $(".upgradeSem-outer").attr('data-depName') ;
    ...
    if(depID != "" && depName != ""){
        ...
        $.ajax({
            cache: false,
            type: "POST",
            url: "/student/upgradesem",
            data: {csrfmiddlewaretoken:$("input[name=csrfmiddlewaretoken]").val(), dep: depID},
            success: function (result) {
                loadingAnim(0) ;
                // console.log(result) ;
                try {
                    let data = JSON.parse(resJson(result))[0] ;
                    if(data == "s"){
                        showLineMsg("Students are upgraded to next semester", "po") ;
                        closeUpgradeSemCont() ;
                    }
                } catch (e) {
                    console.log(e) ;
